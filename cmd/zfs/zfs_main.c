...
us_compare(const void *larg, const void *rarg, void *unused)
{
	...
	for (; sortcol != NULL; sortcol = sortcol->sc_next) {
		...
		case ZFS_PROP_TYPE:
			propname = "type";
			nvlist_lookup_uint32(lnvl, propname, &lv32);
			nvlist_lookup_uint32(rnvl, propname, &rv32);
			if (rv32 != lv32)
				rc = (rv32 > lv32) ? 1 : -1;
			break;
		case ZFS_PROP_NAME:
			propname = "name";
			if (num_name) {
				nvlist_lookup_uint64(lnvl, propname, &lv64);
				nvlist_lookup_uint64(rnvl, propname, &rv64);
				if (rv64 != lv64)
					rc = (rv64 > lv64) ? 1 : -1;
			} else {
				nvlist_lookup_string(lnvl, propname, &lvstr);
				nvlist_lookup_string(rnvl, propname, &rvstr);
...
userspace_cb(void *arg, const char *domain, uid_t rid, uint64_t space)
{
	...
	if (domain == NULL || domain[0] == '\0') {
		...
		sprintf(sid, "%s-%llu", domain, (longlong_t)rid);
		/* SMB */
		if (prop == ZFS_PROP_GROUPUSED || prop == ZFS_PROP_GROUPQUOTA) {
			type = USTYPE_SMB_GRP;
	...
	(void) snprintf(namebuf, sizeof (namebuf), "%llu", (longlong_t)rid);
	if (name == NULL)
		name = namebuf;
	...
	if (cb->cb_nicenum)
		...
		sprintf(sizebuf, "%llu", (longlong_t)space);
	...
	if (cb->cb_numname) {
		if (nvlist_add_uint64(props, "name", (longlong_t)rid) != 0)
			nomem();
		...
		size_t quota_width, us_node_t *node)
{
	...
	while ((nvp = nvlist_next_nvpair(nvl, nvp)) != NULL) {
		...
		case DATA_TYPE_UINT32:
			err = nvpair_value_uint32(nvp, &val32);
			break;
		case DATA_TYPE_UINT64:
			err = nvpair_value_uint64(nvp, &val64);
			break;
		case DATA_TYPE_STRING:
			err = nvpair_value_string(nvp, &strval);
			break;
		default:
		...
		case USFIELD_NAME:
			if (type == DATA_TYPE_UINT64) {
				sprintf(valstr, "%llu", (longlong_t)val64);
				strval = valstr;
			}
			width = name_width;
			break;
...
zfs_do_userspace(int argc, char **argv)
{
	...
	if (resort_avl) {
		...
		for (node = uu_list_first(list); node != NULL;
			...
			us_sort_info_t sortinfo = { sortcol, cb.cb_numname };
			uu_avl_find(avl_tree, node, &sortinfo, &idx);
...
static inline zfs_deleg_note_t
deleg_perm_note(const char *perm)
{
	int i;
	for (i = 0; i < ZFS_NUM_DELEG_NOTES; i++) {
		const char *perm_name = zfs_deleg_perm_tbl[i].z_perm;
		if (strncmp(perm_name, perm, strlen(perm_name)) == 0)
			return (zfs_deleg_perm_tbl[i].z_note);
	}

	return (ZFS_DELEG_NOTE_NONE);
}
...
parse_fs_perm(fs_perm_t *fsperm, nvlist_t *nvl)
{
	...
	while ((nvp = nvlist_next_nvpair(nvl, nvp)) != NULL) {
		...
		parse_who_perm(who_perm, nvl2, perm_locality);
	}
...
allow_usage(boolean_t un, boolean_t requested, const char *msg)
{
	...
	fprintf(fp, gettext("Usage: %s\n"), get_usage(un ? HELP_UNALLOW :
	    HELP_ALLOW));
	fprintf(fp, gettext("Options:\n"));
	for (i = 0; i < (un ? unallow_size : allow_size); i++) {
		const char *opt = opt_desc[i++];
		const char *optdsc = opt_desc[i];
		fprintf(fp, gettext("  %-10s  %s\n"), opt, optdsc);
	}
	fprintf(fp, gettext("\nThe following permissions are supported:\n"));
	fprintf(fp, fmt, gettext("NAME"), gettext("TYPE"), gettext("NOTES"));
	for (i = 0; i < ZFS_NUM_DELEG_NOTES; i++) {
		const char *perm_name = zfs_deleg_perm_tbl[i].z_perm;
		zfs_deleg_note_t perm_note = zfs_deleg_perm_tbl[i].z_note;
		const char *perm_type = deleg_perm_type(perm_note);
		const char *perm_comment = deleg_perm_comment(perm_note);
		fprintf(fp, fmt, perm_name, perm_type, perm_comment);
	}
	...
	for (i = 0; i < count; i++)
		fprintf(fp, fmt, props[i], gettext("property"), "");
    ...
    const char *who, char *perms, nvlist_t *top_nvl)
{
	...
	err = nvlist_alloc(&base_nvl, NV_UNIQUE_NAME, 0);
	err = nvlist_alloc(&set_nvl, NV_UNIQUE_NAME, 0);
	...
	if (perms != NULL) {
		...
		for (i = 0; i < 2; i++) {
			...
			if (!nvlist_empty(base_nvl)) {
				if (who != NULL)
					sprintf(who_buf, "%c%c$%s", base_type,
					    locality, who);
				else
					sprintf(who_buf, "%c%c$", base_type,
					    locality);

				nvlist_add_nvlist(top_nvl, who_buf, base_nvl);
			}


			if (!nvlist_empty(set_nvl)) {
				if (who != NULL)
					sprintf(who_buf, "%c%c$%s", set_type,
					    locality, who);
				else
					sprintf(who_buf, "%c%c$", set_type,
					    locality);

				nvlist_add_nvlist(top_nvl, who_buf, set_nvl);
			}
		}
	} else {
		for (i = 0; i < 2; i++) {
			...
			if (who != NULL)
				...
				sprintf(who_buf, "%c%c$", base_type, locality);
			nvlist_add_boolean(top_nvl, who_buf);

			if (who != NULL)
				sprintf(who_buf, "%c%c$%s", set_type, locality,
				    who);
			else
				sprintf(who_buf, "%c%c$", set_type, locality);
			nvlist_add_boolean(top_nvl, who_buf);
		}
	}
}
...
construct_fsacl_list(boolean_t un, struct allow_opts *opts, nvlist_t **nvlp)
{
	...
	err = nvlist_alloc(nvlp, NV_UNIQUE_NAME, 0);
	if (err)
		nomem();
	...
	if (opts->set) {
		...
		while (curr < end) {
			...
			sprintf(id, "%lu", (unsigned long)rid);
			who = id;
...
print_set_creat_perms(uu_avl_t *who_avl)
{
	...
	for (who_node = uu_avl_first(who_avl); who_node != NULL;
		...
		for (deleg_node = uu_avl_first(avl); deleg_node != NULL;
			...
			if (first) {
				printf("%s", deleg_node->dpn_perm.dp_name);
				first = B_FALSE;
			} else
				printf(",%s", deleg_node->dpn_perm.dp_name);
		}
...
print_fs_perms(fs_perm_set_t *fspset)
{
	...
	for (node = uu_list_first(fspset->fsps_list); node != NULL;
		...
		snprintf(buf, ZFS_MAXNAMELEN+32,
...
zfs_do_allow_unallow_impl(int argc, char **argv, boolean_t un)
{
	...
	if (opts.prt_perms)
		...
		construct_fsacl_list(un, &opts, &update_perm_nvl);
		error = zfs_set_fsacl(zhp, un, update_perm_nvl);
		if (error != 0)
			goto cleanup;

		if (un && opts.recursive) {
			struct deleg_perms data = { un, update_perm_nvl };
			error = zfs_iter_filesystems(zhp, set_deleg_perms,
			    &data);
		}
	}
...
print_holds(boolean_t scripted, int nwidth, int tagwidth, nvlist_t *nvl)
{
	...
	if (!scripted) {
		for (i = 0; i < 3; i++) {
			...
			if (i < 2)
				printf("%-*s  ", i ? tagwidth : nwidth, col);
			else
				printf("%s\n", col);
		}
	}
	...
	while ((nvp = nvlist_next_nvpair(nvl, nvp)) != NULL) {
		...
		while ((nvp2 = nvlist_next_nvpair(nvl2, nvp2)) != NULL) {
			...
			nvpair_value_uint64(nvp2, &val);
			time = (time_t)val;
			localtime_r(&time, &t);
			strftime(tsbuf, DATETIME_BUF_LEN,
...
holds_callback(zfs_handle_t *zhp, void *data)
{
	...
	nvlist_add_nvlist(top_nvl, zname, nvl);
	return (0);
}
